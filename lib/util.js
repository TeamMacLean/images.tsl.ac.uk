...
module.exports = {
    ...
    canAccessGroup: function (groupName, req) {
        ...
        //TODO: fix this; currently disabled I think
        console.log('unexpected reach of code');
        ...
        console.log('req.user', req.user, 'username', req.user.username, 'member of', req.user.memberOf);

        if (!req.user) { //is a user
            return false;
        }

        if ( config.admins && config.admins.length && config.admins.indexOf(req.user.username) > -1) { //is admin
            return true;
        }
